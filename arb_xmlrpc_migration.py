...
class odoo_xmlrpc_migration(object):
    ...
    def get_fields(self, plan, model_name, kwargs):
        ...
        if 'only_fields' in kwargs and kwargs['only_fields'] in field_names:
            logging.debug("Solo migro %s de %s " % (kwargs['only_fields'], model_name))
            field_names = {x: field_names[x] for x in kwargs['only_fields']}
        ...
        for ids in chunk:
            ...
            for row in rows:
                ...
                if len(after_save_fields):
                    ...
                    self.save(plan, data, row['id'], kwargs)
